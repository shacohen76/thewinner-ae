#!/usr/bin/env python3
"""
CG1 Low-Inbound Fix v1.0
=========================
Adds contextual internal links to blog posts that have <2 inbound links.
Each fix inserts a natural sentence with a link into a thematically related post.

Usage:
  python cg1_fix_low_inbound.py              # dry run (shows changes)
  python cg1_fix_low_inbound.py --apply      # applies changes to markdown files
...
# ── Fix map ──────────────────────────────────────────────────────────────────
# Each entry: (target_slug_needing_inbound, host_slug_to_edit, sentence_to_insert)
# The sentence includes a markdown link to the target post.
# We insert after the first ## heading in the host post for natural placement.

FIXES = [
    (
        "how-to-pick-wireless-earbuds",
        "guide-to-buying-next-laptop",
...
def find_insertion_point(content: str) -> int:
    """Find position after the first ## heading (after its line break)."""
    # Find the first ## heading
    match = re.search(r'^## .+$', content, re.MULTILINE)
    if match:
        # Find the end of the paragraph after this heading
        # Look for the next double newline after the heading
        after_heading = match.end()
        next_para_break = content.find('\n\n', after_heading)
        if next_para_break != -1:
            return next_para_break + 2  # after the double newline
    
    # Fallback: insert after first paragraph (first double newline)
...
def main():
    ...
    print("=" * 55)
    print("CG1 Low-Inbound Fix v1.0")
